...
namespace SFML_W{
	...
	void TApplication::Run(SCP_space::SCPattern Chart) {
		...
		int h,v;
		Chart.Get_Size(h,v);
		sf::VertexArray Lines_hb(sf::Lines, (int)(2*(v/10+1)));
		sf::VertexArray Lines_hg(sf::Lines, (int)(2*(v/10*9)));
		sf::VertexArray Lines_vb(sf::Lines, (int)(2*(h/10+1)));
		sf::VertexArray Lines_vg(sf::Lines, (int)(2*(h/10*9)));
